...
function resultVariant(item: BatchLoginItem) {
  return item.status === "成功" ? "success" : item.status === "失败" ? "danger" : "secondary";
}
...
function BatchLoginPageContent() {
  ...
  const progress = job && job.total > 0 ? Math.round((job.processed * 100) / job.total) : 0;
      ...
      try {
        ...
        if (data.job.done) {
          ...
          if (data.job.error) {
            ...
            toast.success(`批量登录完成：成功 ${data.job.success}，失败 ${data.job.fail}`);
          }
        }
      } catch (error) {
        if (closed) return;
        setActiveJobId("");
        toast.error(error instanceof Error ? error.message : "获取批量登录进度失败");
      }
    };
    ...
    <>
      <section className="mb-3 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-1">
          <div className="text-xs font-semibold tracking-[0.18em] text-stone-500 uppercase">Batch Login</div>
          <h1 className="text-2xl font-semibold tracking-tight">批量登录</h1>
        </div>
        <Badge variant="outline" className="w-fit rounded-md border-stone-200 bg-white text-stone-600">
        ...
        <section className="flex min-h-0 flex-col gap-4 border-b border-stone-200 p-4 lg:border-r lg:border-b-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-0.5 text-xs text-stone-500">已识别 {emails.length} 个</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="border border-stone-200 bg-white/80 px-3 py-2">
              ...
              <div className="mt-1 text-lg font-semibold text-stone-900">{job?.total ?? emails.length}</div>
            </div>
            <div className="border border-stone-200 bg-white/80 px-3 py-2">
              <div className="text-xs text-stone-400">成功</div>
              <div className="mt-1 text-lg font-semibold text-emerald-600">{job?.success ?? 0}</div>
            </div>
            <div className="border border-stone-200 bg-white/80 px-3 py-2">
              <div className="text-xs text-stone-400">失败</div>
              <div className="mt-1 text-lg font-semibold text-rose-500">{job?.fail ?? 0}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              {isStarting || isRunning ? <LoaderCircle className="size-4 animate-spin" /> : <LogIn className="size-4" />}
              开始登录
            </Button>
            <Button
              variant="outline"
        ...
        <section className="flex min-h-0 flex-col p-4">
          <div className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h2 className="text-lg font-semibold tracking-tight">任务状态</h2>
                <div className="mt-1 text-sm text-stone-500">
                  {job?.current_email ? `${job.current_email} · ${job.current_step || "处理中"}` : job ? "任务已完成或等待提交" : "等待提交"}
                </div>
              </div>
              <Badge variant={isRunning ? "warning" : job?.done ? "success" : "secondary"} className="w-fit rounded-md">
                {isRunning ? "运行中" : job?.done ? "已完成" : "未开始"}
              </Badge>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-stone-100">
              <div className="h-full bg-stone-950 transition-all" style={{ width: `${progress}%` }} />
            </div>
            ...
            <div className="grid h-full min-h-0 gap-4 xl:grid-cols-[1fr_320px]">
                ...
                <table className="w-full min-w-[720px] text-left">
                  ...
                  <tbody className="divide-y divide-stone-100 text-sm">
                    {(job?.items || []).length === 0 ? (
                      ...
                      job?.items.map((item) => (
                        <tr key={`${item.email}-${item.finished_at || item.status}`}>
                          <td className="break-all px-4 py-3 font-mono text-xs text-stone-700">{item.email}</td>
                          <td className="px-4 py-3">
                            <Badge variant={resultVariant(item)} className="rounded-md">
                              {item.status === "成功" ? <CheckCircle2 className="mr-1 size-3" /> : item.status === "失败" ? <CircleAlert className="mr-1 size-3" /> : null}
                              {item.status}
                            </Badge>
                          </td>
                          <td className="break-all px-4 py-3 font-mono text-xs text-stone-500">{item.token || "--"}</td>
                          <td className={cn("break-words px-4 py-3 text-xs", item.status === "失败" ? "text-rose-600" : "text-stone-500")}>
                            {item.error || item.message || "--"}
                          </td>
                          <td className="px-4 py-3 text-xs text-stone-500">{formatTime(item.finished_at)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div className="min-h-0 overflow-y-auto border border-stone-200 bg-white/80 p-3 font-mono text-xs leading-6">
                {(job?.logs || []).length === 0 ? (
                  ...
                  job?.logs.slice().reverse().map((item, index) => (
                    <div
                      key={`${item.time}-${item.email}-${index}`}
                      className={item.level === "red" ? "text-rose-600" : item.level === "green" ? "text-emerald-700" : "text-stone-700"}
                    >
                      <span className="text-stone-400">{formatTime(item.time)}</span>
