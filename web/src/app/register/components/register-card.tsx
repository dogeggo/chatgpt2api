...
export function RegisterCard() {
  ...
  const setTargetQuota = useSettingsStore(
    (state) => state.setRegisterTargetQuota,
  );
  const setTargetAvailable = useSettingsStore(
    (state) => state.setRegisterTargetAvailable,
  );
  const setCheckInterval = useSettingsStore(
    (state) => state.setRegisterCheckInterval,
  );
  ...
  const updateProvider = useSettingsStore(
    (state) => state.updateRegisterProvider,
  );
  const deleteProvider = useSettingsStore(
    (state) => state.deleteRegisterProvider,
  );
  ...
  const updateProviderType = (index: number, type: string) => {
    updateProvider(index, {
      ...
      ...(type === "cloudflare_temp_email"
        ? { api_base: "", admin_password: "", custom_password: "", domain: [] }
        : {}),
      ...(type === "tempmail_lol" ? { api_key: "", domain: [] } : {}),
      ...(type === "moemail" ? { api_base: "", api_key: "", domain: [] } : {}),
      ...(type === "inbucket"
        ? { api_base: "", domain: [], random_subdomain: true }
        : {}),
      ...(type === "duckmail"
        ? { api_key: "", default_domain: "duckmail.sbs" }
        : {}),
      ...
      ...(type === "yyds_mail"
        ...
        : {}),
      ...(type === "ddg_mail"
        ? { ddg_token: "", cf_inbox_jwt: "", cf_domain: [], admin_password: "" }
        : {}),
    });
  };
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setMode(value as "total" | "quota" | "available")
              }
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setMailField("request_timeout", event.target.value)
                }
                className="h-10 rounded-xl border-stone-200 bg-white"
                disabled={config.enabled}
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setMailField("wait_timeout", event.target.value)
                }
                className="h-10 rounded-xl border-stone-200 bg-white"
                disabled={config.enabled}
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  setMailField("wait_interval", event.target.value)
                }
                className="h-10 rounded-xl border-stone-200 bg-white"
                disabled={config.enabled}
              />
          ...
          <div className="space-y-3">
            {providers.map((provider, index) => {
              ...
              const domains = Array.isArray(provider.domain)
                ? provider.domain.map(String).join("\n")
                : "";
              const subdomains = Array.isArray(provider.subdomain)
                ? provider.subdomain.map(String).join("\n")
                : "";
              return (
                <div
                  key={index}
                    ...
                    <label className="flex items-center gap-3 text-sm text-stone-700">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateProvider(index, { enable: Boolean(checked) })
                        }
                        disabled={config.enabled}
                      />
                      启用
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={type}
                        onValueChange={(value) =>
                          updateProviderType(index, value)
                        }
                        disabled={config.enabled}
                      >
                        <SelectTrigger className="h-10 rounded-xl border-stone-200 bg-white">
                        ...
                        <SelectContent>
                          <SelectItem value="cloudmail_gen">
                            cloudmail_gen
                          </SelectItem>
                          <SelectItem value="cloudflare_temp_email">
                            cloudflare_temp_email
                          </SelectItem>
                          <SelectItem value="tempmail_lol">
                            tempmail_lol
                          </SelectItem>
                          <SelectItem value="moemail">moemail</SelectItem>
                          <SelectItem value="inbucket">
                            inbucket_mail
                          </SelectItem>
                          <SelectItem value="duckmail">duckmail</SelectItem>
                          <SelectItem value="gptmail">
                            gptmail(未测试)
                          </SelectItem>
                          <SelectItem value="yyds_mail">yyds_mail</SelectItem>
                          <SelectItem value="ddg_mail">
                            ddg_mail (DDG邮箱+CF中转)
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      ...
                      <>
                        ...
                        {type === "cloudflare_temp_email" ? (
                          <div className="space-y-2">
                            ...
                            <Input
                              type="text"
                              value={String(provider.custom_password || "")}
                              onChange={(event) =>
                                updateProvider(index, {
                    ...
                    {type === "ddg_mail" ? (
                      <>
                        <div className="space-y-2">
                          <label className="text-sm text-stone-700">
                            DDG Token <span className="text-red-400">*</span>
                          </label>
                          <Input
                            value={String(provider.ddg_token || "")}
                            onChange={(event) =>
                              updateProvider(index, {
                                ddg_token: event.target.value,
                              })
                            }
                            className="h-10 rounded-xl border-stone-200 bg-white"
                            disabled={config.enabled}
                        ...
                        <div className="space-y-2">
                          <label className="text-sm text-stone-700">
                            CF Inbox JWT <span className="text-red-400">*</span>
                          </label>
                          <Input
                            value={String(provider.cf_inbox_jwt || "")}
                            onChange={(event) =>
                              updateProvider(index, {
                                cf_inbox_jwt: event.target.value,
                              })
                            }
                            className="h-10 rounded-xl border-stone-200 bg-white"
                            disabled={config.enabled}
                        ...
                        <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                          ...
                          <ol className="list-decimal list-inside space-y-0.5">
                            ...
                            <li>
                              DDG Token 从浏览器 DevTools → Network →
                              quack.duckduckgo.com 请求中获取{" "}
                    ...
                    {type === "inbucket" ? (
                      <label className="flex items-center gap-3 pt-8 text-sm text-stone-700">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            updateProvider(index, {
                              random_subdomain: Boolean(checked),
                            })
                          }
                          disabled={config.enabled}
                        />
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[
              [
                "成功 / 成功率",
                `${stats.success} / ${stats.success_rate || 0}%`,
              ],
              ["失败", stats.fail],
              ["完成", stats.done],
              ...
              <div
                key={label}
                className="border border-stone-200 bg-white/70 px-3 py-2"
              >
